...
void SetTimer() {
  ...
  while(timertmp == 0){
    if (irrecv.decode(&results)) {  // have we received an IR signal?
      TimeButtonValue = translateIR();
      switch(TimeButtonValue)
        {
          case 15:  // *(Star) pressed, reset time entry.
            timedigits="";
            pos0=0;pos1=0;pos2=0;pos3=0;
            WriteTime();
            timertmp = 1;
            break;
          case 16:  // # pressed - Start timer with number entered
            timertmp = 1;
            break;
          case 0:
            timedigits += TimeButtonValue;
            timelength = timedigits.length();
            if (timelength==1) {
              pos0=0;pos1=0;pos2=0;pos3=timedigits.toInt();WriteTime();
            }
            if (timelength==2) {
              pos0=0;pos1=0;pos2=timedigits.charAt(0);pos3=timedigits.charAt(1);WriteTime();
            }
            if (timelength==3) {
              pos0=0;pos1=timedigits.charAt(0);pos2=timedigits.charAt(1);pos3=timedigits.charAt(2);WriteTime();
            }
            if (timelength==4) {
              pos0=timedigits.charAt(0);pos1=timedigits.charAt(1);pos2=timedigits.charAt(2);pos3=timedigits.charAt(3);
              WriteTime();
            }
            if (timelength==5) {
              pos0=0;pos1=0;pos2=0;pos3=0;
